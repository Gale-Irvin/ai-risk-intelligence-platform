...
EVENT_TEMPLATES = {
    ...
    "SUPPLIER_FAILURE": {
        "title": "Supplier operational failure disrupts supply continuity",
        "scenario": "If a critical supplier experiences operational failure, material or service flow may be disrupted, impacting fulfillment and operational continuity."
    },
    "PHYSICAL_POWER_ENVIRONMENT": {
        "title": "Data center power or environmental failure disrupts operations",
        "scenario": "If power, cooling, or environmental controls fail, critical infrastructure may be disrupted until restoration is completed."
    },
}
...
EVENT_ACTIONS = {
    ...
    "INTEGRATION_FAILURE": (
        "Add alternate transaction paths | Implement queue buffering | Monitor transactions end-to-end",
        "NIST SI-4; NIST CP-2; ISO22301 8.4"
    ),
    "SECURITY_RANSOMWARE": (
        "Harden privileged access | Immutable backups + frequent restore testing | Improve incident response playbooks",
        "NIST IR-4; NIST CP-9; NIST AC-2"
    ),
    "SECURITY_BREACH": (
        "Strengthen third-party security requirements | Improve monitoring/detection | Review data minimization and access controls",
        "NIST AC-3; NIST SI-4; NIST SA-9"
    ),
    "REGULATORY_ENFORCEMENT": (
        "Perform gap assessment against applicable requirements | Document evidence and controls | Track corrective actions to closure",
        "ISO22301; NIST 800-53 (as applicable)"
    ),
    "DESTRUCTIVE_CYBER_ATTACK": (
    "Isolate affected systems, initiate clean-room recovery, validate backups for integrity, and perform forensic analysis before restoration",
    "Cyber Resilience / Destructive Malware"
    ),
    "SUPPLIER_FAILURE": (
        "Qualify alternate suppliers | Improve supplier monitoring and early warning signals | Validate contingency sourcing plans",
        "ISO22301 8.4; NIST CP-2"
    ),
    "PHYSICAL_POWER_ENVIRONMENT": (
        "Test generator/UPS failover | Improve environmental monitoring | Validate facility recovery procedures",
        "NIST PE-11; ISO22301 8.4"
    ),
}
...
def likelihood_baseline(evidence_strength: str, evidence_count: int, conf: str) -> int:
    ...
    # Cap at 5
    return min(likelihood, 5)

def build_exec_summary(cluster_row: pd.Series, evidence_df: pd.DataFrame | None) -> str:
    et = str(cluster_row.get("event_type", "")).strip()
    bf = str(cluster_row.get("business_function", "")).strip()
    asset = str(cluster_row.get("primary_asset_or_service", "")).strip()
    strength = str(cluster_row.get("evidence_strength", "")).strip()
    start = str(cluster_row.get("start_date", "")).strip()
    end = str(cluster_row.get("end_date", "")).strip()
    count = int(cluster_row.get("evidence_count", 0) or 0)

    headline = f"Risk signal ({strength}): {et} affecting {asset} in {bf}."
    timing = f"Evidence window: {start or 'N/A'} to {end or 'N/A'} across {count} source(s)."
    ...
    if evidence_df is not None and not evidence_df.empty:
        ...
        subset = evidence_df[evidence_df["evidence_id"].astype(str).isin(ev_ids)]
...
def main():
    ...
    print(df_clusters[df_clusters["cluster_id"].isin(["CL-000124","CL-000129","CL-000175","CL-000179","CL-000184"])][["cluster_id","primary_asset_or_service","event_type"]])
    evidence_df = None
    try:
    ...
    for _, c in df_clusters.iterrows():
        ...
        event_type = str(c.get("event_type", "")).strip() or "UNKNOWN"
        business_function = str(c.get("business_function", "")).strip() or "Unknown Function"
        primary_asset = str(c.get("primary_asset_or_service", "")).strip() or "UNKNOWN"
        evidence_ids = str(c.get("evidence_ids", "")).strip()

        # Risk identifiers
        risk_id = stable_risk_id(event_type, primary_asset, business_function)
        ...
        imp = EVENT_TO_IMPACT.get(event_type, 3)
        score = like * imp

        # Impact dimensions
        availability, financial, regulatory, reputation, safety = EVENT_TO_DIMENSIONS.get(event_type, (4, 3, 2, 3, 1))

        # Actions + mappings
        actions, mapping = EVENT_ACTIONS.get(event_type, ("Review controls and implement appropriate mitigations", "N/A"))

        impact_narr = f"{event_type} affecting {primary_asset} can disrupt {business_function}, causing operational delays and recovery complications."
        exec_summary = build_exec_summary(c, evidence_df)

        score_rationale = (
            f"Derived from incident cluster {cluster_id}. Evidence strength={strength}, evidence_count={evidence_count}, "
            f"confidence={conf}. Baseline likelihood/impact assigned using deterministic rubric for {event_type}."
        )

        out_rows.append({
            # --- Tightened join fields ---
            "risk_id": risk_id,
            ...
            "assets_or_services": primary_asset,
            "scenario": scenario,
            ...
            # --- Internal enrichment placeholders ---
            ...
            # --- Narrative + controls placeholders ---
    ...
    template_cols = [
        "risk_id","cluster_id","event_type","primary_asset_or_service","evidence_ids",
        "title","risk_category","business_function","assets_or_services","scenario",
        "evidence_strength","likelihood_baseline","impact_baseline","risk_score_baseline",
        "internal_signals_used","internal_outage_refs","likelihood_adjusted","impact_adjusted","risk_score_adjusted",
        "confidence","availability_impact","financial_impact","regulatory_impact","reputation_impact","safety_impact",
